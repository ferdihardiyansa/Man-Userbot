...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(
            "`fer-Userbot Berhasil Di Deploy! Userbot bisa di gunakan kembali.`"
        )
...
async def update(event, repo, ups_rem, ac_br):
    try:
        ...
        repo.git.reset("--hard", "FETCH_HEAD")
    await event.edit("`fer-Userbot Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`")
...
async def upstream(event):
    ...
    if conf == "deploy":
        await event.edit("`[HEROKU]: Update Deploy fer-Userbot Sedang Dalam Proses...`")
        await deploy(event, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await event.edit("**✥ fer-Userbot Sudah Versi Terbaru**")
