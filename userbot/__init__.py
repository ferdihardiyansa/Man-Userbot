...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL", "https://github.com/ferdihardiyansa/ferubot.git"
)
UPSTREAM_REPO_BRANCH = os.environ.get("UPSTREAM_REPO_BRANCH", "fer-Userbot")
        ...
        async def inline_handler(event):
            ...
            if event.query.user_id == uid and query.startswith("@SharingUserbot"):
                ...
                result = builder.photo(
                    file=logoman,
                    link_preview=False,
                    text=f"**✗ fer-Userbot Inline Menu ✗**\n\n✣ **Owner** [{user.first_name}](tg://user?id={user.id})\n✣ **Jumlah** `{len(dugmeler)}` Modules",
                    buttons=buttons,
                )
